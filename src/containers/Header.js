...
class Header extends Component {
  ...
  render() {
    ...
    if (this.state.showComunas) {
      ...
      let farmaciasComunas = this.farmaciasDeTurno.filter(
        farmacia => farmacia.fk_region == numeroRegion,
      );
      ...
      farmaciasComunas.sort((a, b) =>
        a.comuna_nombre < b.comuna_nombre ? -1 : a.comuna_nombre > b.comuna_nombre ? 1 : 0,
      );
      ...
      comunas = farmaciasComunas.map((farmacia, index) => {
        ...
        if (comunasList.includes(farmacia.comuna_nombre)) {
          ...
          return (
            <option key={index} value={farmacia.comuna_nombre}>
